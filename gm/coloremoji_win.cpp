...
class ColorEmojiWinGM : public GM {
public:
    ColorEmojiWinGM() {
        fTypeface = NULL;
    }
...
protected:
    void onOnceBeforeDraw() SK_OVERRIDE {
        fTypeface = SkTypeface::CreateFromName("Segoe UI Emoji", SkTypeface::kNormal);
    }

    SkString onShortName() SK_OVERRIDE {
        return SkString("coloremojiwin");
    }
    ...
    void onDraw(SkCanvas* canvas) SK_OVERRIDE {
        ...
        for (size_t y = 0; y < sizeof(textSize) / sizeof(int); y++) {
            ...
            canvas->drawText(text, textLen, 10, SkIntToScalar(textYOffset[y]), paint);
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                for (int k = 0; k < 2; k++) {
                    ...
                    canvas->drawText(text, textLen, 380, SkIntToScalar(y_offset), shaderPaint);
                    y_offset += 32;
                }
            }
        }
        ...
        SkRect bounds;
        paint.measureText(text, textLen, &bounds);
...
static GM* MyFactory(void*) { return new ColorEmojiWinGM; }
