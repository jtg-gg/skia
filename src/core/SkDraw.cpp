...
public:
    ...
    if (glyph.fColorLayer) { // this is to handle the color glyph
        for (const SkGlyph::ColorRun* colorRun = glyph.fColorLayer->begin(); colorRun != glyph.fColorLayer->end(); colorRun++) {
            const SkGlyph* i = &fGlyphCache->getGlyphIDMetrics(colorRun->fNextGlyphId, glyph.getSubXFixed(), glyph.getSubYFixed());
            ...
            if (NULL == aa) {
                aa = (uint8_t*)fGlyphCache->findImage(*i);
                if (NULL == aa) {
                    continue; // can't rasterize glyph
                }
            }
            mask.fImage = aa;
            SkARGB32_Opaque_Blitter* blitter = static_cast<SkARGB32_Opaque_Blitter*>(fBlitter);
